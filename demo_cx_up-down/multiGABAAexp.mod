TITLE multiple GABA-A receptors
...
Model of GABA-A receptors: "exponential" model modified from a first-order
kinetic model with pulse of transmitter (see Destexhe, A., Mainen, Z. and
Sejnowski, T.J.  Neural Computation, 6: 14-18, 1994).  The parameters were
obtained from fitting the model to whole-cell recorded GABA-A postsynaptic
currents (Otis et al, J. Physiol.  463: 391-407, 1993).  The fit was performed
using a simplex algorithm using short pulses of transmitter (0.5 mM during 0.3
ms).
...
c = new multiGABAAexp()		// create multiple GABA-A kinetic synapses
...
DEFINE MAXSYNGABAA 250
VERBATIM
static int MAXSYNGABAA = 250;
ENDVERBATIM

INDEPENDENT {t FROM 0 TO 1 WITH 1 (ms)}

NEURON {
	POINT_PROCESS multiGABAAexp
...
:VERBATIM
:    if(id == -228) {
:	printf("GABA: id, t, i = %2.2f\t%2.2f\t%2.4f\n", id, t, i);
:    }
:ENDVERBATIM
}
